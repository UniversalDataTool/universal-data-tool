export default ({ youtubeUrl, title: videoTitle, remote, videoQuality, downloadPath, onProgress, onComplete, overallProgress }) => {
    let starttime = Date.now()
    
    const ytdl = remote.require("ytdl-core")
    const path = remote.require("path")
    
    const videoName = videoQuality.includes("audio") ? `${videoTitle + ".mp3"}` : `${videoTitle + ".mp4"}`
